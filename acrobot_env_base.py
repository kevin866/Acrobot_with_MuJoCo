...
class AcrobotMujocoEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.current_episode += 1  # <- increment episode here
    ...
    def step(self, action):
        ...
        # Curriculum Learning: scale wind strength based on training progress
        progress = min(self.current_episode / self.total_episodes, 1.0)
        wind_strength = progress * 1.0  # 1.0 = max wind scale
        wind_force = np.array([np.random.normal(0.0, 1.0) * wind_strength, 0, 0, 0, 0, 0])
